...
const SKILLS = {
    ...
    // ================================================================
    // SHINJUKU ITADORI — ELO 3200 MAX
    // ================================================================
    'Prince of Black Flash': { cost: 0, type: 'passive', color: '#ff2d55', slot: 'Ability', desc: 'PASSIVE — Shinjuku Itadori\'s Black Flash mastery. 60% Black Flash chance on every attack. After 20 turns, "OPPI" is ready: automatically fires when opponent activates or has an active domain — collapses their domain, blocks their domain slot for 10 turns, and skips their next turn. OPPI recharges every 20 turns after use.' },
    'Ultimate Dark Flash': { cost: 600, type: 'target', color: '#ff2d55', slot: 'Ultimate', desc: 'ULTIMATE — A Black Flash straight to the soul. Click a square adjacent (orthogonally) to any friendly piece. Deals + shaped damage (target + 4 adjacent squares). All damage is soul damage — cannot be regenerated by RCT for 10 turns. Counts as a Black Flash: grants 200 CE and helps recover burnouts. Cost: 600 CE.' },
    'Cursed Existence': { cost: 600, type: 'instant', color: '#ff2d55', slot: 'Domain', desc: 'DOMAIN EXPANSION [Level 1] — Yuji\'s soul made manifest. While active: 100% Black Flash chance on every attack, all damage is permanent soul damage (never regenerable by RCT), and ALL skills have infinite range (can target any square regardless of distance or line of sight). Lasts 10 turns. Clashes with other Level 1 domains. Collapses against Level 2 or 3 domains. Cost: 600 CE.' },
    'Regenerate CT': { cost: 60, type: 'instant', color: '#00ff88', slot: 'CT Regeneration', desc: 'CT REGENERATION — Spend 60 CE to recover 10% of burnout bar. If in burnout, skip it instantly for 600 CE. WARNING: After skipping 3 burnouts or healing 200% total, your nose bleeds... After that, 10-turn burnout and permanent domain block. Unlock by defeating any MAX character.' },
...
let state = {
    ...
    // Shinjuku Itadori state
        ...
        // Hakari Kinji state
        hakariDomainActive: false, hakariDomainTimer: 0,
        hakariJackpotActive: false, hakariJackpotTurns: 0,
        hakariSpinCooldown: 0, hakariPseudoGetoUsed: false,
...
function startBattle(name, elo) {
    // IMMEDIATELY clear all domain visuals and chat from previous battle
    try {
        ...
        if (_gs) {
            _gs.classList.remove('sukuna-domain','infinite-void-domain','heian-domain','malevolent-domain',
                ...
                'simple-domain-active','heian-domain','tml-tcmp-clash','tml-sep-clash','tcmp-sep-clash',
...
// OPPI helper: called after any domain is activated
function checkOPPITrigger(turn) {
    if (state.opp === 'Shinjuku Itadori' && state.oppiReady && state.turn === turn) {
        ...
        if (state.infiniteVoidActive) { state.infiniteVoidActive = false; state.infiniteVoidTimer = 0; document.getElementById('void-veil').style.display = 'none'; }
        if (state.gojoVoidActive) { state.gojoVoidActive = false; state.gojoVoidTimer = 0; document.getElementById('void-veil').style.display = 'none'; }
        if (state.playerHeianDomainActive) { state.playerHeianDomainActive = false; document.getElementById('heian-veil').style.display = 'none'; }
        if (state.heianDomainActive) { state.heianDomainActive = false; document.getElementById('heian-veil').style.display = 'none'; }
        state.playerTMLActive = false; document.getElementById('tml-veil').style.display = 'none';
        state.playerSEPActive = false; document.getElementById('sep-veil').style.display = 'none';
        state.playerTCMPActive = false; document.getElementById('tcmp-veil').style.display = 'none';
        state.playerCSGActive = false; state.playerCSGTimer = 0; document.getElementById('csg-veil').style.display = 'none';
...
function executeTech(name, isAI, r, c) {
    ...
    if (name === 'Infinity') {
        ...
        let placedAt = null;
        if (restoredType === 'P') {
            // Try back row first, then any empty square on player's half
            for (let cc = 0; cc < 8; cc++) { if (!state.board[backRow2][cc]) { placedAt = [backRow2, cc]; break; } }
            if (!placedAt) {
                // Try any empty square (including IFG row)
                for (let rr = isAI ? 0 : 4; isAI ? rr < 4 : rr < 8; isAI ? rr++ : rr++) {
                    for (let cc = 0; cc < 8; cc++) { if (!state.board[rr][cc]) { placedAt = [rr, cc]; break; } }
                    if (placedAt) break;
                }
            }
            ...
            if (!placedAt) {
                // Try any empty square on player's half
                for (let rr = isAI ? 0 : 4; isAI ? rr < 4 : rr < 8; isAI ? rr++ : rr++) {
                    for (let cc = 0; cc < 8; cc++) { if (!state.board[rr][cc]) { placedAt = [rr, cc]; break; } }
                    if (placedAt) break;
                }
        ...
        if (isAI && state.infP > 0 && state.mahoragaAdaptedLimitless) showMahoragaWheel();
            ...
            log('Infinity: Straw Doll blocked!');
        ...
        if (!isAI && state.infE > 0 && !canBypassBarrier(false)) {
            log("Infinity: Straw Doll blocked by opponent's Infinity!");
            state.ceP += getTechCost(name); state.casting = null; return;
        }
        ...
        // CHANGE 2: targets ANY enemy piece except King and Mahoraga, anywhere
        const target = state.board[r]?.[c];
        if (target && target.color !== color && target.type !== 'K' && !target.isAdaptive) {
            ...
            log('Cleave: select a valid enemy piece (not King or Mahoraga).');
            state.ceP += getTechCost(name); state.casting = null; return;
        }
        ...
        if (!isAI) {
            ...
            if (state.infP > 0 && !canBypassBarrier(true)) {
                log('Infinity: Hollow Purple blocked!');
    ...
    if (name === 'Heian Cleave') {
        ...
        if (isAI) {
            ...
            for (let dr = -3; dr <= 3; dr++) for (let dc = -3; dc <= 3; dc++) {
                ...
                if (p?.color === 'W' && p.type !== 'K') {
                    if (p.isAdaptive) { state.playerMahoragaActive = false; state.mahoragaDestroyed = true; state.playerAdaptedTech = []; state.playerTechUsageCount = {}; state.mahoragaAdaptedLimitless = false; state.mahoragaLimitlessBlocks = 0; state.playerMahoragaDomainAdaptTimer = 0; showTitle('MAHORAGA DESTROYED', '#FF4444'); }
            ...
            log('Domain Expansion: Malevolent Shrine: Hean! The true form awakens — three cuts every two turns!');
            // OPPI check
            if (state.opp === 'Shinjuku Itadori' && state.oppiReady && state.turn === 'W') {
                state.oppiReady = false; state.oppiCooldown = 20;
                // OPPI: anti-domain strikes — visual only
        ...
        if (!isAI) {
            ...
            if (state.infE > 0 && !canBypassBarrier(false)) {
                log("Gojo's Infinity: Hollow Nuke blocked!");
    ...
    // ================================================================
    // SHINJUKU ITADORI SKILLS
    // ================================================================

    // --- Simple Domain ---
    if (name === 'Simple Domain') {
        if (!isAI) {
            if (state.simpleDomainCooldown > 0) {
                log('Simple Domain: ' + state.simpleDomainCooldown + ' turns cooldown remaining.');
                state.ceP += getTechCost(name); state.casting = null; return;
            }
            if (state.simpleDomainActive) {
                log('Simple Domain is already active!');
                state.ceP += getTechCost(name); state.casting = null; return;
            }
            ...
            // AI Simple Domain
            if (state.simpleDomainCooldown > 0 || state.simpleDomainActive) {
                state.ceE += getTechCost(name, true); if (!state._aiNoEndTurn) endTurn(); return;
            }
            // AI uses Simple Domain when enemy has domain active
    ...
    // --- Cursed Existence (Level 1 Domain) ---
    if (name === 'Cursed Existence') {
        if (!isAI) {
            // Check if enemy has higher level domain
            const myLvl = 1; // Level 1
            const enemyLvl = enemyDomainLevel();
            if (enemyLvl > myLvl) {
                log('Cursed Existence collapses — overwhelmed by the stronger domain!');
                showTitle('COLLAPSED', '#ff2d55');
                state.ceP += getTechCost(name); state.casting = null; render(); return;
            }
            // Check arm requirement
            if (!hasBothArms('W')) {
    ...
    // --- Pseudo Geto (Hakari guaranteed jackpot) ---
    if (name === 'Pseudo Geto') {
        if (!isAI) {
            if (state.hakariPseudoGetoUsed) {
                log('Pseudo Geto already used this battle!');
                state.ceP += getTechCost(name); state.casting = null; return;
            }
            if (!state.hakariDomainActive) {
                log('Pseudo Geto requires Idle Death Gamble to be active!');
                state.ceP += getTechCost(name); state.casting = null; return;
            }
    ...
    // --- Door of Death (Hakari pachinko door trap) ---
    if (name === 'Door of Death') {
        if (!isAI) {
            if (r === undefined || c === undefined) {
                state.ceP += getTechCost(name); state.casting = null; return;
            }
            if (state.board[r][c]) {
                log('Door of Death requires an empty square!');
                state.ceP += getTechCost(name); state.casting = null; return;
            }
            if (state.hakariDoors.length >= 2) {
    ...
    // --- Bagara: Kazan (Fire Storm) ---
    if (name === 'Bagara: Kazan') {
        if (!isAI) {
            if (r === undefined || c === undefined) {
                state.ceP += getTechCost(name); state.casting = null; return;
            }
    ...
    if (name === 'Ultimate Dark Flash') {
        if (!isAI) {
            // Check adjacency to friendly piece
            const hasFriendly = state.board[r][c]?.color === 'W';
            if (hasFriendly) {
            ...
            for (const [dr,dc] of dirs) {
                ...
                if (nr>=0 && nr<8 && nc>=0 && nc<8 && state.board[nr]?.[nc]?.color === 'W') {
                    adjacentToFriendly = true; break;
                }
            }
            ...
            for (const [tr,tc] of targets) {
                ...
                if (p && p.color === 'B' && p.type !== 'K') {
                    ...
                    // Mark as soul damage (can't be RCT'd for 10 turns)
                    state.soulDamage = state.soulDamage || [];
                    state.soulDamage.push({ type: p.type, turnsLeft: 10 });
                    if (p.isAdaptive) {
                        state.playerMahoragaActive = false; state.mahoragaDestroyed = true;
                        state.playerAdaptedTech = []; state.playerTechUsageCount = {};
                        state.mahoragaAdaptedLimitless = false; state.mahoragaLimitlessBlocks = 0;
                        state.playerMahoragaDomainAdaptTimer = 0;
                        showTitle('MAHORAGA DESTROYED', '#FF4444');
                    }
                    state.board[tr][tc] = null;
                    playAnim(tr, tc, 'dark-flash-anim');
                    hitCount++;
                }
            }
            // Grant Black Flash benefits
            state.ceP = Math.min(state.ceMaxP, state.ceP + 200);
            state.blackFlashCount = Math.max(0, state.blackFlashCount - 2);
            if (state.rctBurnoutTurns > 0) state.rctBurnoutTurns = Math.max(0, state.rctBurnoutTurns - 2);
            if (state.domainBurnoutTurns > 0) state.domainBurnoutTurns = Math.max(0, state.domainBurnoutTurns - 2);
            state.blackFlashIntensity = 3;
            ...
            for (let tr = 0; tr < 8; tr++) for (let tc = 0; tc < 8; tc++) {
                ...
                if (score > bestScore) { bestScore = score; bestTarget = { r: tr, c: tc }; }
            }
            if (bestTarget && bestScore >= 3) {
                const targets_ai2 = [[bestTarget.r,bestTarget.c],[bestTarget.r-1,bestTarget.c],[bestTarget.r+1,bestTarget.c],[bestTarget.r,bestTarget.c-1],[bestTarget.r,bestTarget.c+1]];
                let hitCount2 = 0;
                for (const [tr,tc] of targets_ai2) {
                    if (tr<0||tr>8||tc<0||tc>8) continue;
                    const p = state.board[tr]?.[tc];
                    if (p && p.color === 'W' && p.type !== 'K') {
                        state.capturedByE.push(p.type);
                ...
                state.ceE = Math.min(state.ceEMax, state.ceE + 200);
                state.blackFlashIntensity = 3;
                showTitle('ULTIMATE DARK FLASH', '#ff2d55');
                shakeScreen(); impactFlash('rgba(255,45,85,.6)', 300);
    ...
    const _noEndTurn = ['Reverse Cursed Technique', 'Idle Transfiguration', 'Infinity',
        ...
        'Time Cell Moon Palace', 'Chimera Shadow Garden', 'Simple Domain', 'Cursed Existence', 'Regenerate CT'];
...
function handleCellClick(r, c) {
    ...
    if (state.lapseBluePhase === 'move') {
        ...
        // Block: cannot teleport to back ranks (prevents pawn promotion bug)
        if (r === 0 || r === 7) { log('Lapse Blue: cannot teleport to back rank!'); render(); return; }
...
function applyMove(fr, fc, tr, tc, m) {
    ...
    if (target || m.isEP) {
        ...
        if (p.color === 'W' && !hrBypass && !playerHasDomain && state.infE > 0 && !isDomainClash()) {
            log("Infinity Blocked! (lasts until end of this turn)"); return true;
    ...
    if (state.naoyaTCMPActive && !isDomainClash()) {
        ...
        if (_placed?.color === 'W' && _placed.type !== 'K' && !_placed.isPlayerRika && !_placed.isAdaptive) {
...
function getActiveDomainLevels_onSide(isPlayer) {
    ...
    if (isPlayer) {
            ...
            if (state.hakariDomainActive && state.hakariJackpotOwner === 'W') level = Math.max(level, 2);
        ...
        if (state.domain2?.owner === 'W') {
            ...
            if (state.hakariDomainActive && state.hakariJackpotOwner === 'B') level = Math.max(level, 2);
...
function onBlackFlash(color) {
    ...
    // Enhanced visual for 3x intensity
    if (state.blackFlashIntensity >= 3) {
        state.blackFlashIntensity = 1; // Reset after use
        // Create full-screen flash overlay
        const flash = document.createElement('div');
        flash.style.cssText = 'position:fixed;inset:0;z-index:9999;pointer-events:none;background:radial-gradient(circle,#fff 0%,#8b00ff 15%,#ff2d55 30%,rgba(0,0,0,.95) 60%,transparent 80%);animation:black-flash-blast-3x .9s forwards;box-shadow:0 0 60px rgba(255,45,85,.6),0 0 120px rgba(139,0,255,.4);';
        document.body.appendChild(flash);
        // Screen shake
        document.body.style.animation = 'shake-hard .5s';
        ...
        case 'Hollow Purple': {
            ...
            for (let dc = 0; dc <= 1; dc++) for (let r = 0; r < 8; r++) {
                ...
                if (p && p.color === enemyColor && p.type !== 'K') {
                    state[capturedKey].push(p.type); state.board[r][bestCol + dc] = null; playAnim(r, bestCol + dc, 'cleave-anim');
...
function endTurn() {
    ...
    // Tick soul damage
    if (state.soulDamage && state.soulDamage.length > 0) {
    ...
    if (_hasActiveDomain) {
        ...
        if (state.domainDuration >= 20) {
            ...
            const wasGojoVoid = state.gojoVoidActive;
            const wasHeianP = state.playerHeianDomainActive;
            const wasHeianE = state.heianDomainActive;
            const wasDomain = state.domain;
            const wasDomain2 = state.domain2;
            // Remove all domain CSS classes
            if (gs2) gs2.classList.remove('sukuna-domain', 'infinite-void-domain', 'heian-domain', 'malevolent-domain', 'tml-domain', 'sep-domain', 'tcmp-domain', 'csg-domain', 'domain-clash', 'tml-tcmp-clash', 'tml-sep-clash', 'tcmp-sep-clash', 'sep-clash', 'tml-tml-clash', 'sep-sep-clash', 'tcmp-tcmp-clash');
            // Deactivate all domain state
            if (wasPlayerVoid) { state.infiniteVoidActive = false; state.infiniteVoidTimer = 0; document.getElementById('void-veil').style.display = 'none'; }
            if (wasGojoVoid) { state.gojoVoidActive = false; state.gojoVoidTimer = 0; document.getElementById('void-veil').style.display = 'none'; }
            if (wasHeianP) { state.playerHeianDomainActive = false; document.getElementById('heian-veil').style.display = 'none'; }
            if (wasHeianE) { state.heianDomainActive = false; document.getElementById('heian-veil').style.display = 'none'; }
            state.playerTMLActive = false; document.getElementById('tml-veil').style.display = 'none';
            state.playerSEPActive = false; document.getElementById('sep-veil').style.display = 'none';
            state.playerTCMPActive = false; document.getElementById('tcmp-veil').style.display = 'none';
            state.playerCSGActive = false; state.playerCSGTimer = 0; document.getElementById('csg-veil').style.display = 'none';
            ...
            const _ceVC = document.getElementById('ce-veil');
            if (_ceVC) _ceVC.style.display = 'none';
            // Remove CE domain CSS
            if (gs2) gs2.classList.remove('cursed-existence-domain');
            triggerDomainBurnout();
        }
    }
    ...
    if (state.turn === 'W') {
        ...
        // Imaginary Fierce God passive: grants one extra move per player turn
                if (Object.values(prog.eq).includes('Imaginary Fierce God') && !isDivinaSealed('Imaginary Fierce God') && !isDomainClash())
                    state.extraMovesThisTurn = Math.max(state.extraMovesThisTurn, 1);
                // Hakari jackpot: +1 extra move per turn while jackpot active
                if (state.hakariJackpotActive && state.hakariJackpotOwner === 'W' && !isDomainClash())
                    state.extraMovesThisTurn = Math.max(state.extraMovesThisTurn, 1);
...
function aiCycle(isSecondMove = false) {
    ...
    if (state.opp === 'Naoya Zenin' && !isDomainClash() && (state.ceE >= 80 || state.naoyaTCMPActive)) {
        if (!state.naoyaTCMPActive) state.ceE -= 80;
    ...
    // ================================================================
    // SHINJUKU ITADORI AI (ELO 3200 MAX)
    // Priority: Ultimate Dark Flash → Cursed Existence → Simple Domain
    // ================================================================
    if (state.opp === 'Shinjuku Itadori' && !isDomainClash()) {
        // Tick OPPI cooldown
        if (state.oppiCooldown > 0) state.oppiCooldown--;
        ...
        if (!_udfBlocked && state.ceE >= 600) {
            ...
            if (bestTarget && bestScore >= 3) {
                const targets_udf2 = [[bestTarget.r,bestTarget.c],[bestTarget.r-1,bestTarget.c],[bestTarget.r+1,bestTarget.c],[bestTarget.r,bestTarget.c-1],[bestTarget.r,bestTarget.c+1]];
                let hitCount = 0;
                for (const [tr,tc] of targets_udf2) {
                    if (tr<0||tr>8||tc<0||tc>8) continue;
                    const p = state.board[tr]?.[tc];
                    if (p && p.color === 'W' && p.type !== 'K') {
                        state.capturedByE.push(p.type);
        ...
        // PRIORITY 3: Simple Domain (400 CE) when enemy has domain
        if (state.ceE >= 400 && !state.simpleDomainActive && state.simpleDomainCooldown <= 0) {
            const enemyHasDomain = state.gojoVoidActive || state.infiniteVoidActive || state.heianDomainActive ||
                state.playerHeianDomainActive || state.trueMutualLoveActive || state.mahitoDomainActive ||
                state.naoyaTCMPActive || state.csgActive || state.playerTMLActive || state.playerSEPActive ||
                state.playerTCMPActive || state.playerCSGActive || state.aiCursedExistenceActive || state.playerCursedExistenceActive;
        ...
        if (bestCapture) {
            ...
            if (pieces.length) {
                const s = pieces[Math.floor(Math.random() * pieces.length)];
...
function render() {
    ...
    let hasPlayerMahoraga = false;
    let hasAIMahoraga = false;
    for (let r = 0; r < 8; r++) {
        for (let c = 0; c < 8; c++) {
            const p = state.board[r][c];
            if (p) {
                if (p.isPlayerRika) hasPlayerRika = true;
                if (p.isRika && p.color === 'B') hasYutaRika = true;
                if (p.isAdaptive && p.color === 'W') hasPlayerMahoraga = true;
                if (p.isAdaptive && p.color === 'B') hasAIMahoraga = true;
            }
        }
    }
    if (state.playerRikaActive && !hasPlayerRika) {
        state.playerRikaActive = false;
        state.playerRikaDestroyed = true;
        log("Your Rika was defeated!");
    }
    if (state.yutaRikaActive && !hasYutaRika) {
        state.yutaRikaActive = false;
        state.yutaRikaDestroyed = true;
        log("Rika defeated — Yuta's costs return to normal.");
    }
    if (state.playerMahoragaActive && !hasPlayerMahoraga) {
        state.playerMahoragaActive = false;
        state.mahoragaDestroyed = true;
        state.playerAdaptedTech = [];
        state.playerTechUsageCount = {};
        ...
        state.mahoragaLimitlessBlocks = 0;
        state.playerMahoragaDomainAdaptTimer = 0;
        log("Your Mahoraga was destroyed!");
    }
    if (state.mahoragaActive && !hasAIMahoraga) {
        state.mahoragaActive = false;
        state.mahoragaDestroyed = true;
        state.adaptedTech = [];
        state.mahoragaDomainAdaptTimer = 0;
        state.mahoragaAdaptedLimitless = false;
        state.mahoragaLimitlessBlocks = 0;
        log("Enemy Mahoraga was destroyed!");
    }
